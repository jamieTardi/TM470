...
export const createBooking = async (
	...
	try {
		...
		if (!currentCustomer || !currentLake || !currentUser) {
			return res
				.status(401)
				.json({ message: 'Customer or lake or user not found' });
		}
		...
		const newBooking = await bookings.create({
			...
			datesBooked: eachDayOfInterval({
				start: new Date(
					new Date(arrival).setHours(new Date(arrival).getHours() + 6),
				),
				end: new Date(
					new Date(departure).setHours(new Date(departure).getHours() - 6),
				),
			}),
			emailsSent: 0,
			placesBooked,
		...
		return res
			.status(401)
			.json({ message: 'An error processing the booking has happened' });
	}
};
...
export const checkAvailability = async (req: Request, res: Response) => {
	...
	if (!currentLake) {
		res.status(400).json({
			message: 'Cannot find a lake, please make sure one is selected.',
		});
	}
	try {
		...
		if (unavaliableArray.length > 0) {
			if (customerId === unavaliableArray[0].customerId) {
				return res.status(200).json({
					message: `This is the current booking for the selected customer.`,
				});
			}
			...
			return res.status(200).json({
				message: `
					...
					<span class="bold">Arrival Date: </span> ${format(
						new Date(arrival),
						'eeee do MMMM yyyy',
					)}  
					</br> 
					</br> 
					<span class="bold">Departure Date: </span> ${format(
						new Date(departure),
						'eeee do MMMM yyyy',
					)}`,
			});
		...
		return res.status(500).json({
			message:
				'There was an error getting that booking date. Please try again.',
		});
	}
};

export const sendInfoEmail = async (req: any, res: Response) => {
	const { customerId, lakeId, placesBooked, refNo, cost, paid, arrival, _id } =
		req.body.booking;
	...
	try {
		...
		const currentCustomer = await customer.findById(
			options ? options.customerId : customerId,
		);
		...
		if (!options) {
			...
			await bookings.findByIdAndUpdate(_id, {
				bookingEmailSent: currentBooking?.bookingEmailSent
					? currentBooking?.bookingEmailSent + 1
					: 1,
			});
		}
		...
		if (!currentLake || !currentCustomer) {
			res.status(401).json({
				message:
					'There has been an error finding that lake or customer, please try again or contact the developer.',
			});
		}
		...
		fs.readFile(
			'emails/bookingEmail.html',
			{ encoding: 'utf-8' },
			function (err, html) {
				if (err) {
					return res.status(500).json({
						message:
							'An error happened while trying to send the email. Please refresh and try again.',
					});
				} else {
					...
					let data = {
						...
						arrival: format(
							new Date(options ? options.arrival : arrival),
							'eeee do MMMM yyyy',
						),
						departure: format(
							new Date(options ? options.departure : arrival),
							'eeee do MMMM yyyy',
						),
					...
					transporter.sendMail(mailOptions, function (error, info) {
						if (error) {
							return res.status(500).json({
								message:
									'The email failed to send, try refreshing or sending again.',
							});
						} else {
							console.log('Email sent: ' + info.response);
							return res
								.status(200)
								.json('Booking confirmation email sent successfully. 📧');
						}
					});
				}
			},
		...
		res.status(500).json({
			message: 'The email failed to send, try refreshing or sending again.',
		});
	}
};
...
export const getAllBookingsForYear = async (req: Request, res: Response) => {
	...
	const allBookings = await bookings.find({
		$and: [
			{ datesBooked: { $gte: new Date(year) }, $lt: new Date(year + 1) },
			{ lakeId: lakeId },
		],
	});
		...
		.map((el) => {
			switch (el.date.getMonth()) {
				case 0:
					sortedDates = {
						...sortedDates,
						jan: [...sortedDates.jan, { date: el.date, places: el.places }],
					};
					break;
				case 1:
					sortedDates = {
						...sortedDates,
						feb: [...sortedDates.feb, { date: el.date, places: el.places }],
					};
					break;
				case 2:
					sortedDates = {
						...sortedDates,
						mar: [...sortedDates.mar, { date: el.date, places: el.places }],
					};
					break;
				case 3:
					sortedDates = {
						...sortedDates,
						apr: [...sortedDates.apr, { date: el.date, places: el.places }],
					};
					break;
				case 4:
					sortedDates = {
						...sortedDates,
						may: [...sortedDates.may, { date: el.date, places: el.places }],
					};
					break;
				case 5:
					sortedDates = {
						...sortedDates,
						june: [...sortedDates.june, { date: el.date, places: el.places }],
					};
					break;
				case 6:
					sortedDates = {
						...sortedDates,
						july: [...sortedDates.july, { date: el.date, places: el.places }],
					};
					break;
				case 7:
					sortedDates = {
						...sortedDates,
						aug: [...sortedDates.aug, { date: el.date, places: el.places }],
					};
					break;
				case 8:
					sortedDates = {
						...sortedDates,
						sept: [...sortedDates.sept, { date: el.date, places: el.places }],
					};
					break;
				case 9:
					sortedDates = {
						...sortedDates,
						oct: [...sortedDates.oct, { date: el.date, places: el.places }],
					};
					break;
				case 10:
					sortedDates = {
						...sortedDates,
						nov: [...sortedDates.nov, { date: el.date, places: el.places }],
					};
					break;
				case 11:
					sortedDates = {
						...sortedDates,
						dec: [...sortedDates.dec, { date: el.date, places: el.places }],
					};
					break;
			}
		});
