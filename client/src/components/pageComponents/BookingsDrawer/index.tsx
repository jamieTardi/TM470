...
const BookingsDrawer = () => {
	...
	const currentBookings: TBooking[] = useSelector<RootState>((state) => state.bookingsState.bookings) as TBooking[];
	const currentUser: IUser = useSelector<RootState>((state) => state.auth.user) as IUser;
	const bookingId = searchParams.get("bookingId");
	const existingBooking: TBooking | undefined = currentBookings
		? currentBookings.filter((booking: TBooking) => booking._id === bookingId)[0]
		: undefined;
	...
	const handleSendForm = useCallback(
		(e: React.FormEvent<HTMLFormElement>) => {
			...
			if (!existingBooking) {
				createBooking(booking, handleClearLoading, isSendEmail, currentUser._id!);
				return;
			}
		...
		<form onSubmit={(e) => handleSendForm(e)}>
			<Drawer>
				<DrawerHeader handleClose={handleClearParams}>
					{existingBooking ? `Edit booking for ${arrival}` : "Create a new booking"}
				</DrawerHeader>
				<Content
					booking={booking}
				...
				{showModal && (
					...
					>
						Are you Sure you want to delete this booking? This will <span className="bold">PERMANENTLY</span> delete
						this booking and all of their associated information?
					</Modal>
				)}
			</Drawer>
		</form>
